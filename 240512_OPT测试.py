...
#  CONSTRAINT_Engry_rest) = MCD_OPT_CORE(MCD_AM,MCD_FRE_A,MCD_SPAN,ID_MOTOR,MCD_GEAR_RATIO,MCD_S_AR,MCD_TR,
...
#                  P_core_HOVER,R_ref,Individual_Differences_HOVER,FAST_inter,
#                  CENTER_SP_1, CENTER_SP_2, CENTER_SP_3, CENTER_SP_4)
# print("AIO-单次评估耗时:", (time.time() - start_time))
#
# print("仿生隐居距离",EVAL_MCD)
# print("悬停时间",EVAL_T_HOVER)
# print("RUSH后悬停时间",EVAL_T_MISSION)
# print("最大速度",EVAL_V_MAX)
# if CONSTRAINT_motor_percent<0.5:
#     IS_OK_CONSTRAINT_motor_percent = True
# else:
#     IS_OK_CONSTRAINT_motor_percent = False
# print("IS_OK_CONSTRAINT_motor_percent",IS_OK_CONSTRAINT_motor_percent,"---",CONSTRAINT_motor_percent)
#
#
# if CONSTRAINT_maneuvering>1:
#     IS_OK_CONSTRAINT_maneuvering = True
# else:
#     IS_OK_CONSTRAINT_maneuvering = False
# print("IS_OK_CONSTRAINT_maneuvering",IS_OK_CONSTRAINT_maneuvering,"---",CONSTRAINT_maneuvering)
#
# if CONSTRAINT_Engry_rest>0:
#     IS_OK_CONSTRAINT_Engry_rest = True
# else:
#     IS_OK_CONSTRAINT_Engry_rest = False
# print("IS_OK_CONSTRAINT_Engry_rest",IS_OK_CONSTRAINT_Engry_rest,"---",CONSTRAINT_Engry_rest)
...
seq_MCD_AM = np.linspace(40, 85, num)
seq_MCD_FRE_A = np.linspace(10, 65, num)
seq_MCD_SPAN = np.linspace(0.05, 0.15, num)
...
for i in range(num):
    for j in range(num):
        for k in range(num):
            print("===================================================================================================")
            MCD_AM =seq_MCD_AM[i]
            MCD_FRE_A =seq_MCD_FRE_A[j]
            MCD_SPAN =seq_MCD_SPAN[k]
            ...
            (EVAL_MCD, \
             ...
             CONSTRAINT_Engry_rest,OK_FLAP) = MCD_OPT_CORE(MCD_AM, MCD_FRE_A, MCD_SPAN, ID_MOTOR, MCD_GEAR_RATIO, MCD_S_AR,
                                                   MCD_TR,
                                                   NDE_phi_1, NDE_phi_2, NDE_phi_3, NDE_phi_4, NDE_theta_1, NDE_theta_2,
            ...
            if IS_OK_CONSTRAINT_motor_percent==False or IS_OK_CONSTRAINT_maneuvering==False or IS_OK_CONSTRAINT_Engry_rest==False or OK_FLAP == False:
                print("MCD_AM", MCD_AM)
                print("MCD_FRE_A", MCD_FRE_A)
                print("MCD_SPAN", MCD_SPAN)
                print("仿生隐居距离", EVAL_MCD)
                print("悬停时间", EVAL_T_HOVER)
                print("RUSH后悬停时间", EVAL_T_MISSION)
                print("最大速度", EVAL_V_MAX)
