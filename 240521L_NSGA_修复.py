...
# def create_initial_population(n_individuals, feasible_solution, bounds):
#     X = np.empty((n_individuals, len(feasible_solution)))
#     for i in range(n_individuals):
#         perturbation = feasible_solution * 0.05 * np.random.uniform(low=-1, high=1, size=feasible_solution.shape)
#         X[i, :] = feasible_solution + perturbation
#         X[i, :] = np.clip(X[i, :], bounds[0], bounds[1])  # 保证个体在边界内
#     return Population.new("X", X)

# 定义可行解和边界
# feasible_solution = np.array([80, 34, 0.075, 3, 25])
# bounds = np.array([[10, 15, 0.05, -0.5, 5], [85, 50, 0.12, 20.49, 35]])
# initial_population = create_initial_population(200, feasible_solution, bounds)
# # 定义参考方向
