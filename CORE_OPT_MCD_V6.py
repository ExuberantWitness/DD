...
from CONTROL_PIDcontrol_V7 import PIDNN
...
from CORE_MOTOR_MODEL_V3 import GET_MOTOR_DATA_V2,ELE_MOTOR,NEST_INT
...
MCD_AM_MAX_LIFT = 90
GENE_SCALE = 4
CONST_INIT_P = 24.0e-03 * GENE_SCALE * 1.1
CONST_INIT_I = 0.1e-05 * GENE_SCALE *0.9
CONST_INIT_D = 35e-06 * GENE_SCALE * 1.1
CONST_E_trans = 1 # 目前假设机械传动效率为90%，参见《Untethered Flight of an At-Scale Dual-Motor Hummingbird Robot With Bio-Inspired Decoupled Wings》
MAX_TORQUE_SCALE = 2
SPRING_SCALE = 1
...
LR_PID = 0.2
TOL_PID = 5 / 57.1  # 如果误差在2度内就不更新PID
...
def MCD_OPT_CORE(MCD_AM,MCD_FRE_A,MCD_SPAN,ID_MOTOR,MCD_GEAR_RATIO,
    ...
    :return:
    """
    P_mean = 0 # 防止语法错误,给一个随便值
    MEAN_P1 = 0 # 防止语法错误,给一个随便值
    MEAN_P2 = 0 # 防止语法错误,给一个随便值
    MEAN_P3 = 0 # 防止语法错误,给一个随便值
    MEAN_P4 = 0 # 防止语法错误,给一个随便值
    REST_HOVER_s = -100
    T_endurance = 0
    V = -100
    MAX_LIFT = -100
    ...
    # %% 电机参数读取和基本惯量参数
    U_motor, I_zero, I_max, R_motor, KV_motor, W_max, MASS_MOTOR, INT_MOTOR, P_heat_MAX = GET_MOTOR_DATA_V2(NEST_INT(ID_MOTOR))
    I_max = I_max * MAX_TORQUE_SCALE
    MCD_J_M_ZZ = INT_MOTOR/10000000 *MCD_GEAR_RATIO**2
    ...
    J_W_XX,J_W_YY,J_W_ZZ,J_W_XY,J_W_XZ,J_W_YZ = Inertia_WING(S_total = S_total,
                                                             ...
                                                             C_root = C_root,
                                                             c_tip = c_tip)
    J_W_ZZ_NEW = SHIFT_J(J_W_XX, J_W_YY, J_W_ZZ, J_W_XY, J_W_XZ, J_W_YZ) * SPRING_SCALE # 调整这个刚度系数,对于功耗特性影响非常明显,越好功耗越小

    CONST_J = MCD_J_M_ZZ + J_GEAR  # 这个仅仅是电机轴,不考虑翼的影响
    ...
    QSM_W3 = AERO_QS_FLY(C,
                         ...
                         rho,
                         CENTER = CENTER_SP_3)
    QSM_W4 = AERO_QS_FLY(C,r,Re,AR, Ro,N_blade,dr,DUM_XC_c_mean_tr_ratio, CORE_F_rot2, P_S_cop_rot, P_C_cop_rot,
                         DUM_XC_c_mean_tr, DUM_RC_R2_tr,rho, CENTER = CENTER_SP_4)
    ...
    # %% 自适应PID控制器初始化
    # pid_controller_5 =PIDNN(P_VALUE =CONST_INIT_P, I_VALUE =CONST_INIT_I, D_VALUE =CONST_INIT_D, learning_rate =LR_PID, tolerance = TOL_PID, timestep =const_DT)
    # pid_controller_6 =PIDNN(P_VALUE =CONST_INIT_P, I_VALUE =CONST_INIT_I, D_VALUE =CONST_INIT_D, learning_rate =LR_PID, tolerance = TOL_PID, timestep =const_DT)
    # pid_controller_7 =PIDNN(P_VALUE =CONST_INIT_P, I_VALUE =CONST_INIT_I, D_VALUE =CONST_INIT_D, learning_rate =LR_PID, tolerance = TOL_PID, timestep =const_DT)
    # pid_controller_8 =PIDNN(P_VALUE =CONST_INIT_P, I_VALUE =CONST_INIT_I, D_VALUE =CONST_INIT_D, learning_rate =LR_PID, tolerance = TOL_PID, timestep =const_DT)
    ...
    # %% 主体组件
    Inertia_characteristics = [CONST_J, J_W_XX, J_W_YY, J_W_ZZ, J_W_XY, J_W_XZ, J_W_YZ]
    ...
    # %% 多次迭代恒定值计算
    N_K_A1 = MCD_FRE_A ** 2 * 4 * np.pi ** 2 * (CONST_J + J_W_ZZ_NEW)
    N_K_A2 = MCD_FRE_A ** 2 * 4 * np.pi ** 2 * (CONST_J + J_W_ZZ_NEW)
    N_K_A3 = MCD_FRE_A ** 2 * 4 * np.pi ** 2 * (CONST_J + J_W_ZZ_NEW)
    N_K_A4 = MCD_FRE_A ** 2 * 4 * np.pi ** 2 * (CONST_J + J_W_ZZ_NEW)
    N_CONTROL_SAFW = 0.0
    N_CONTROL_SAHW = 0.0

    # %% 正式循环执行
    DATA = np.zeros((const_NUMB, 28))
    ...
    # for i_step in tqdm(range(const_NUMB)):
        ...
        # N_TORQUE_M_1 = pid_controller_5.predict(LAST_O_STATE[4] , LAST_O_STATE[0])  # 用两个状态
        # N_TORQUE_M_2 = pid_controller_6.predict(LAST_O_STATE[5] , LAST_O_STATE[1])
        # N_TORQUE_M_3 = pid_controller_7.predict(LAST_O_STATE[6] , LAST_O_STATE[2])
        # N_TORQUE_M_4 = pid_controller_8.predict(LAST_O_STATE[7] , LAST_O_STATE[3])
        ...
        P_MOTOR_1, U_motor_1, I_motor_1, P_HEAT1 = ELE_MOTOR(W_MOTOR_1, T_MOTOR_1, R_motor, I_zero, KV_motor)
        P_MOTOR_2, U_motor_2, I_motor_2, P_HEAT2 = ELE_MOTOR(W_MOTOR_2, T_MOTOR_2, R_motor, I_zero, KV_motor)
        P_MOTOR_3, U_motor_3, I_motor_3, P_HEAT3 = ELE_MOTOR(W_MOTOR_3, T_MOTOR_3, R_motor, I_zero, KV_motor)
        P_MOTOR_4, U_motor_4, I_motor_4, P_HEAT4 = ELE_MOTOR(W_MOTOR_4, T_MOTOR_4, R_motor, I_zero, KV_motor)
        ...
        DATA[i_step, 16] = P_HEAT1
        DATA[i_step, 17] = P_HEAT2
        DATA[i_step, 18] = P_HEAT3
        DATA[i_step, 19] = P_HEAT4

        DATA[i_step, 20] = (LAST_O_STATE[4] - LAST_O_STATE[0]) * 57.1
        DATA[i_step, 21] = (LAST_O_STATE[5] - LAST_O_STATE[1]) * 57.1
        DATA[i_step, 22] = (LAST_O_STATE[6] - LAST_O_STATE[2]) * 57.1
        DATA[i_step, 23] = (LAST_O_STATE[7] - LAST_O_STATE[3]) * 57.1

        DATA[i_step, 24] = (positions[4]) * 57.1
        DATA[i_step, 25] = (positions[5]) * 57.1
        DATA[i_step, 26] = (positions[6]) * 57.1
        DATA[i_step, 27] = (positions[7]) * 57.1
    ...
    ERROR_1 = np.mean(np.abs(NEW_DATA[:, 20]))
    ERROR_2 = np.mean(np.abs(NEW_DATA[:, 21]))
    ERROR_3 = np.mean(np.abs(NEW_DATA[:, 22]))
    ERROR_4 = np.mean(np.abs(NEW_DATA[:, 23]))

    # print("跟踪误差",ERROR_1,ERROR_2,ERROR_3,ERROR_4)
    ...
    if MAX_W_MOTOR_1<W_max and MAX_W_MOTOR_2<W_max and MAX_W_MOTOR_3<W_max and MAX_W_MOTOR_4<W_max:
        ...
        if MAX_I_MOTOR_1 < I_max and MAX_I_MOTOR_2 < I_max and MAX_I_MOTOR_3 < I_max and MAX_I_MOTOR_4 < I_max:
            ...
            MEAN_HEAT_1 = np.mean(NEW_DATA[:, 16])
            MEAN_HEAT_2 = np.mean(NEW_DATA[:, 17])
            MEAN_HEAT_3 = np.mean(NEW_DATA[:, 18])
            MEAN_HEAT_4 = np.mean(NEW_DATA[:, 19])


            if MEAN_HEAT_1<P_heat_MAX and MEAN_HEAT_2<P_heat_MAX and MEAN_HEAT_3<P_heat_MAX and MEAN_HEAT_4<P_heat_MAX:
                ...
                if PERCENT_MOTOR<0.35: # 这里要约束成30%
                    ...
                    if MCD_AM<89:
                        ...
                        # for i_step in tqdm(range(const_NUMB)):
                            ...
                            DATA[i_step, 20] = theta_inst_0
                            DATA[i_step, 21] = theta_inst_1
                            DATA[i_step, 22] = theta_inst_2
                            DATA[i_step, 23] = theta_inst_3
                            ...
                            if MAX_I_MOTOR_1 < I_max and MAX_I_MOTOR_2 < I_max and MAX_I_MOTOR_3 < I_max and MAX_I_MOTOR_4 < I_max:
                                ...
                                if MAX_LIFT>TAKE_OFF_res:
                                    ...
                                    if MEAN_HEAT_1<P_heat_MAX and MEAN_HEAT_2<P_heat_MAX and MEAN_HEAT_3<P_heat_MAX and MEAN_HEAT_4<P_heat_MAX:
                                        ...
                                        # print("平均热功率", MEAN_HEAT_1, MEAN_HEAT_2, MEAN_HEAT_3, MEAN_HEAT_4)
                                        # print("平均效率",
                                        #       1-MEAN_HEAT_1/MEAN_P1_ML,
                                        #       1-MEAN_HEAT_2/MEAN_P2_ML,
                                        #       1-MEAN_HEAT_3/MEAN_P3_ML,
                                        #       1-MEAN_HEAT_4/MEAN_P4_ML)

                                        P_total_ML = MEAN_P1_ML + MEAN_P2_ML + MEAN_P3_ML + MEAN_P4_ML + COUNT_P_etc

                                        # %% 迎风面积计算
                                        S_wing = (c_tip + C_root) * (S_total - D_dc)
                                        S_FRONT = (MCD_GEAR_RATIO + 1) * Gear_Motor * M_gear * C_root

                                        S = S_FRONT * 2 + S_wing * 2# 本来应该除2的
    ...
    P_mean = (MEAN_P1 + MEAN_P2 + MEAN_P3 + MEAN_P4)/4

    return EVAL_MCD,EVAL_T_HOVER,EVAL_T_MISSION,EVAL_V_MAX,FUXX_FLAP,P_mean