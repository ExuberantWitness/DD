...
def DDD_SYSTEM(CENTER_SP_1,CENTER_SP_2,CENTER_SP_3,CENTER_SP_4,J_M_ZZ = 0.000000497, J_W_XX = 0.00000027, J_W_YY = 0.00000003, J_W_ZZ = 0.00000024, J_W_XY = 0.0,J_W_XZ = 0.0, J_W_YZ = 0.00000006):
    ...
    sympy_tuple = tuple(LIST_FREEDOM + LIST_FREEDOM_d + ARG_CONTROL + ARG_AERO + ARG_ELASTIC)
...
def SPRING_sigmoid_SHAPE(theta, SCACLE_GEM=0.1):
    ...
    KE_VALUE_ref = (((1.35 - (1 / (sigma * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((np.radians(REF_ANGLE) - mu) / sigma) ** 8)) / 1.35) ** 2 - ((1.35 - (1 / (sigma * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((0 - mu) / sigma) ** 8)) / 1.35) ** 2)
