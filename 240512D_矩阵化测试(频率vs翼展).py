...
ARRAY = np.zeros((LEN_MCD_FRE_A,LEN_MCD_SPAN,4))

ORIGIN_EVAL_MCD = np.zeros((LEN_MCD_FRE_A,LEN_MCD_SPAN))
ORIGIN_EVAL_T_HOVER =np.zeros((LEN_MCD_FRE_A,LEN_MCD_SPAN))
ORIGIN_EVAL_T_MISSION =np.zeros((LEN_MCD_FRE_A,LEN_MCD_SPAN))
ORIGIN_EVAL_V_MAX =np.zeros((LEN_MCD_FRE_A,LEN_MCD_SPAN))

for i in range(LEN_MCD_FRE_A):
    for j in range(LEN_MCD_SPAN):
        MCD_FRE_A = seq_MCD_FRE_A[i]
...
filename = f'{current_time}OBJ_MCD_V_MAX_60.csv'
df.to_csv(filename, index=False)
