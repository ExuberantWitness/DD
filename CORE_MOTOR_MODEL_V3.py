...
def GET_MOTOR_DATA_V2(ID_MOTOR):
    ...
    # 电机的总热阻
    S_MOTOR = MOTOR_array[ID_MOTOR, 26]/1000 * MOTOR_array[ID_MOTOR, 1]/1000 * np.pi
    S_stand = 0.000175
    K_sink = 9.88
    K_heat = MOTOR_array[ID_MOTOR, 25] + K_sink * S_stand/S_MOTOR

    P_heat_MAX = (125-25)/K_heat


    return U_motor, I_zero, I_max, R_motor, KV_motor, W_max,MASS_MOTOR,INT_MOTOR,P_heat_MAX
...
def ELE_MOTOR(W_MOTOR, T_MOTOR,R_motor,I_zero,KV_motor):
    ...
    P_MOTOR = U_motor * I_motor + U_motor*I_zero * SCALE_NP
    P_HEAT = R_motor * I_motor**2
    return P_MOTOR, U_motor, I_motor, P_HEAT

def NEST_INT(num):
    return round(num)


U_motor, I_zero, I_max, R_motor, KV_motor, W_max,MASS_MOTOR,INT_MOTOR,P_heat_MAX = GET_MOTOR_DATA_V2(0)
