...
def linear_interp(x, xp, fp):
    ...
    for i in range(len(x)):
        if x[i] <= xp[0]:
            ...
            for j in range(n-1):
                if xp[j] <= x[i] < xp[j+1]:
                    result[i] = fp[j] + (fp[j+1] - fp[j]) * (x[i] - xp[j]) / (xp[j+1] - xp[j])
                    break
    return result
...
spec = [
    ...
    ('num_blade', int64),
    ('dr', float64),
    ('DUM_XC_c_mean_tr_ratio', float64),
    ('CORE_F_rot2', float64),
    ('P_S_cop_rot', float64),
    ('P_C_cop_rot', float64),
    ('CENTER', float64),
    ('DUM_XC_c_mean_tr', float64),
    ('DUM_RC_R2_tr', float64),
    ('POS_x_B', float64),
    ('POS_y_B', float64),
    ('POS_z_B', float64),
    ('theta_B_x', float64),
...
class AERO_QS_FLY:
    def __init__(self,
        ...
        self.dr = dr
        self.DUM_XC_c_mean_tr_ratio = DUM_XC_c_mean_tr_ratio
        self.CORE_F_rot2 = CORE_F_rot2
        self.P_S_cop_rot = P_S_cop_rot
        self.P_C_cop_rot = P_C_cop_rot
        self.CENTER = CENTER
        self.DUM_XC_c_mean_tr = DUM_XC_c_mean_tr
        self.DUM_RC_R2_tr = DUM_RC_R2_tr
    ...
    def MFD_Force_translation_Discrete_DICK(self,
        ...
        SUM_W_FX_tr, SUM_W_FY_tr, SUM_W_FZ_tr, \
        SUM_W_MX_tr, SUM_W_MY_tr, SUM_W_MZ_tr, \
        SUM_B_L_tr, SUM_B_D_tr = MFD_Force_translation_Discrete_DICK_out(self.Re,
                                                                         self.AR,
                                                                         self.Ro,
                                                                         self.rho,
                                                                         self.num_blade,
    ...
    def MFD_Force_rotation_REDICK(self, theta_inst, theta_b_dot_inst, PHI_b_dot_inst, INTERACTION_RATIO):
        ...
        FX_rot = F_rot2 * INTERACTION_RATIO
    ...
    def AERO_SOLVER_FLY(self,
        ...
        FL_B_tr_DICK, FD_B_tr_DICK = self.MFD_Force_translation_Discrete_DICK(PHI_inst,
                                                                              ...
                                                                              theta_inst,
                                                                              theta_b_dot_inst,
                                                                              self.POS_x_B, self.POS_y_B, self.POS_z_B,
                                                                              self.theta_B_x, self.theta_B_y, self.theta_B_z,
                                                                              self.w_x_B, self.w_y_B, self.w_z_B,
                                                                              self.VX_ref_I, self.VY_ref_I, self.VZ_ref_I,
                                                                              INTERACTION_RATIO)
        ...
        DRY_NP = R_xyz_NP(0.0, -theta_inst, -PHI_inst + self.CENTER)
        B_B_TOL = np.dot(DRY_NP.T, F_W_TOL)
        FX_B_TOL = B_B_TOL[0]
        FY_B_TOL = B_B_TOL[1]
        FZ_B_TOL = B_B_TOL[2]

        MX_W_TOL = MX_W_tr_DICK + MX_W_rot
        MY_W_TOL = MY_W_tr_DICK + MY_W_rot
        MZ_W_TOL = MZ_W_tr_DICK + MZ_W_rot

        FL_B_TOL = FL_B_tr_DICK + FL_B_rot
        FD_B_TOL = FD_B_tr_DICK + FD_B_rot

        return FX_B_TOL, FY_B_TOL, FZ_B_TOL, \
               MX_W_TOL, MY_W_TOL, MZ_W_TOL, \
               FL_B_TOL, FD_B_TOL, self.AOA
